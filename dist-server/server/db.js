...
class ServerDB {
    ...
    static async getUserByEmail(email) {
        ...
        if (pool) {
            const res = await pool.query(`SELECT u.id, u.email, u.password_hash, u.auth_provider, u.provider_account_id, p.full_name, p.avatar_url, 'USER' as role, u.created_at, u.updated_at
         FROM users u
    ...
    static async getUserByGoogleId(googleId) {
       ...
       FROM users u
       LEFT JOIN profiles p ON u.id = p.user_id
       WHERE u.auth_provider = 'google' AND u.provider_account_id = $1 LIMIT 1;`, [googleId]);
        if (res.rows.length === 0)
            return null;
        const row = res.rows[0];
        const isSuperAdmin = ["seamafridi123456789@gmail.com", "khalid@tradingos.io"].includes(row.email.toLowerCase());
    ...
    static async resolveOrCreateGoogleUser(profile) {
        const pool = this.getPool();
        if (!pool)
        ...
        if (existingByGoogleId) {
            if (profile.picture || profile.name) {
                await pool.query(`UPDATE profiles SET full_name = COALESCE($1, full_name), avatar_url = COALESCE($2, avatar_url), updated_at = NOW() WHERE user_id = $3;`, [isSuperAdmin ? "Seam Afridi (Super Admin)" : (profile.name || null), profile.picture || null, existingByGoogleId.id]);
            }
        ...
        if (existingByEmail) {
            await pool.query(`UPDATE users SET auth_provider = 'google', provider_account_id = $1, email_verified = TRUE, updated_at = NOW() WHERE id = $2;`, [cleanSub, existingByEmail.id]);
            if (profile.picture) {
                await pool.query(`UPDATE profiles SET avatar_url = COALESCE(avatar_url, $1), updated_at = NOW() WHERE user_id = $2;`, [profile.picture, existingByEmail.id]);
            }
            return {
                user: {
                    ...existingByEmail,
        ...
        try {
            ...
            await client.query(`INSERT INTO users (id, email, password_hash, auth_provider, provider_account_id, email_verified, created_at, updated_at)
         VALUES ($1, $2, NULL, 'google', $3, TRUE, NOW(), NOW())
         ON CONFLICT (id) DO UPDATE SET auth_provider = 'google', provider_account_id = EXCLUDED.provider_account_id, email_verified = TRUE, updated_at = NOW();`, [userId, cleanEmail, cleanSub]);
            await client.query(`INSERT INTO profiles (id, user_id, full_name, avatar_url, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())
         ON CONFLICT (user_id) DO UPDATE SET full_name = EXCLUDED.full_name, avatar_url = COALESCE(profiles.avatar_url, EXCLUDED.avatar_url), updated_at = NOW();`, [`prof_${userId}`, userId, fullName, profile.picture || null]);
         ...
         ON CONFLICT (user_id) DO NOTHING;`, [`sub_${userId}`, userId, tier, isSuperAdmin ? "System Owner (Lifetime)" : "Direct"]);
            await client.query(`INSERT INTO trading_accounts (id, user_id, name, broker, account_type, currency, balance, equity, is_default, created_at, updated_at)
         VALUES ($1, $2, 'Primary Quant Account', 'Binance Futures Feed', 'LIVE', 'USD', $3, $3, TRUE, NOW(), NOW())
         ON CONFLICT (id) DO NOTHING;`, [`acc_${userId}_primary`, userId, initialCapital]);
         ...
         ON CONFLICT (id) DO NOTHING;`, [`plan_${userId}`, userId]);
         ...
         ON CONFLICT (user_id) DO NOTHING;`, [`risk_${userId}`, userId]);
