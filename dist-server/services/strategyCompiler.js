...
const strategyValidator_1 = require("./strategyValidator");
class StrategyCompiler {
    static DEFAULT_API_KEY_STORAGE_KEY = "trading_os_gemini_api_key";
    static getApiKey() {
        try {
     ...
     */
    static async compile(promptText, symbol = "BTCUSDT", timeframe = "15m") {
    ...
    static checkAmbiguity(text) {
        ...
        const hasMeasurableRule = /\b(candle|candles|rsi|ema|sma|macd|bollinger|supertrend|vwap|stoch|stochastic|high|low|swing|breakout|engulfing|support|resistance|session|london|tp|sl|stop|target|profit|loss|%)\b/i.test(lower);
     ...
     */
    static async callGeminiStructuredAPI(promptText, apiKey, symbol, timeframe) {
...
Output strictly valid JSON matching this exact structure:
{
  ...
  "riskManagement": {
    "riskPerTrade": 1.0
        ...
        const validation = strategyValidator_1.StrategyValidator.validate(parsed);
     ...
     */
    static compileOffline(promptText, symbol = "BTCUSDT", timeframe = "15m") {
        ...
        if (candleSeq) {
            ...
            const val = strategyValidator_1.StrategyValidator.validate(astObj);
            if (!val.valid || !val.ast)
     ...
     */
    static parseCandleSequence(text) {
        ...
        if (has3Candles || (hasGreenRedPattern && (text.includes("next") || text.includes("previous")))) {
            ...
            if (text.includes("short") && !text.includes("long"))
                dir = "SHORT";
            else if (text.includes("long") && !text.includes("short"))
                dir = "LONG";
            return {
                direction: dir,
                entry: {
     ...
     */
    static parseTechnicalConditions(text) {
        const conditions = [];
        const defaultParams = {};
        ...
        if (text.includes("session") || text.includes("london") || text.includes("new york") || text.includes("asian")) {
            ...
            else if (text.includes("asian") || text.includes("tokyo"))
                sessionName = "Asian";
            conditions.push({
                type: "session",
                session: sessionName
        ...
        if (!hasIndicator || conditions.length === 0)
            return null;
        const isOr = text.includes(" or ");
        const compositeCondition = conditions.length === 1
            ? conditions[0]
            : {
                type: isOr ? "OR" : "AND",
                conditions
        ...
        const compositeShortCondition = shortConditions.length === 1
            ? shortConditions[0]
            : {
                type: isOr ? "OR" : "AND",
     ...
     */
    static generatePineScriptFromAST(ast, symbol = "BTCUSDT", timeframe = "15m") {
...
// ==========================================
// LONG: ${strategyValidator_1.StrategyValidator.formatCondition(ast.entry.long)}
longCondition = ${this.generatePineCondition(ast.entry.long, true)}

// SHORT: ${strategyValidator_1.StrategyValidator.formatCondition(ast.entry.short)}
...
`;
    ...
    static generatePineCondition(cond, isLong = true) {
        if (!cond)
            return "false";
        if (cond.type === "AND") {
        ...
        switch (cond.type) {
            case "candle": {
                ...
                if (cond.property === "bullish")
                    return `close[${offset}] >= open[${offset}]`;
                if (cond.property === "bearish")
                    return `close[${offset}] < open[${offset}]`;
                return `close[${offset}] >= open[${offset}]`;
            }
            case "indicator": {
            ...
            case "price": {
                return `close ${cond.operator} ta.ema(close, ${cond.reference?.params?.period || 200})`;
            }
            default:
