...
exports.RuleViolationEngine = void 0;
class RuleViolationEngine {
    /**
     * Audit all trades against active Trading Plan and Risk constraints
     */
    static auditTrades(trades, plan, risk, accountEquity = 10000) {
        const violations = [];
        ...
        for (let i = 0; i < sortedTrades.length; i++) {
            ...
            if (trade.session && plan.allowedSessions && plan.allowedSessions.length > 0) {
                const isAllowed = plan.allowedSessions.some(s => s.toLowerCase().includes(trade.session.toLowerCase()) || trade.session.toLowerCase().includes(s.toLowerCase()));
                if (!isAllowed) {
                    violations.push({
...
exports.RuleViolationEngine = RuleViolationEngine;
