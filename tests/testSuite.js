import { StrategyCompiler } from "../src/services/strategyCompiler.js";
import { StrategyValidator } from "../src/services/strategyValidator.js";
import { ASTEvaluator } from "../src/services/astEvaluator.js";
...
async function runAllTests() {
  ...
  await test("Test 0: 3-Candle Sequence Strategy (Section 22 Verification)", async () => {
    ...
    assert(ast.entry.long.conditions.length === 3, "Long condition must have 3 candle conditions");
    assert(ast.entry.long.conditions[0].candleOffset === -2 && ast.entry.long.conditions[0].property === "bullish", "t-2 must be bullish");
    assert(ast.entry.long.conditions[1].candleOffset === -1 && ast.entry.long.conditions[1].property === "bullish", "t-1 must be bullish");
    assert(ast.entry.long.conditions[2].candleOffset === 0 && ast.entry.long.conditions[2].property === "bearish", "t-0 must be bearish");

    // Check Short condition structure
    assert(ast.entry.short.type === "AND", "Short condition must be an AND logical group");
    assert(ast.entry.short.conditions.length === 3, "Short condition must have 3 candle conditions");
    assert(ast.entry.short.conditions[0].candleOffset === -2 && ast.entry.short.conditions[0].property === "bearish", "t-2 must be bearish");
    assert(ast.entry.short.conditions[1].candleOffset === -1 && ast.entry.short.conditions[1].property === "bearish", "t-1 must be bearish");
    assert(ast.entry.short.conditions[2].candleOffset === 0 && ast.entry.short.conditions[2].property === "bullish", "t-0 must be bullish");
  ...
  await test("Test E: Logical OR (RSI < 30 OR Bullish Engulfing)", async () => {
    ...
    assert(ast.entry.long.conditions.length === 2, "Must contain 2 branch conditions");
  });
