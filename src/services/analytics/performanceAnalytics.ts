...
import { Trade, PerformanceMetrics } from "../../types/domain";

export class PerformanceAnalytics {
  /**
   * Compute comprehensive performance metrics across a list of trades
   */
  static calculate(trades: Trade[], initialCapital = 10000): PerformanceMetrics {
   ...
   */
  static breakdownBySession(trades: Trade[]): Record<string, { trades: number; winRate: number; netPnl: number }> {
    const sessions = ["London", "New York", "Asian", "Overlap"];
    const breakdown: Record<string, { trades: number; winRate: number; netPnl: number }> = {};
