...
export class ServerDB {
  ...
  private static getDbUrl(): string | null {
  ...
  static isPostgresActive(): boolean {
    this.getPool();
    ...
    if (pool) {
      ...
      try {
        ...
        await client.query(
          ...
          [`prof_${user.id}`, user.id, user.fullName]
        );
        ...
        await client.query(
          ...
          [`plan_${user.id}`, user.id, "Trading-OS Master Trading Constitution"]
        );
        ...
        await client.query(
          ...
          [`risk_${user.id}`, user.id]
        );

        await client.query(
          `INSERT INTO trading_accounts (id, user_id, name, broker, account_type, currency, balance, equity, is_default)
           VALUES ($1, $2, 'Main Quant Execution Account', 'Institutional Interbank Feed', 'LIVE', 'USD', 10000.00, 10000.00, TRUE)
           ON CONFLICT (id) DO NOTHING;`,
          [`acc_${user.id}_primary`, user.id]
        );

        await client.query(
          `INSERT INTO subscriptions (id, user_id, tier, status, provider, current_period_start)
           VALUES ($1, $2, 'FREE', 'ACTIVE', 'Direct', NOW())
           ON CONFLICT (user_id) DO NOTHING;`,
          [`sub_${user.id}`, user.id]
        );
  ...
  static async getTrades(userId: string): Promise<Trade[]> {
    ...
    const res = await pool.query(
      `SELECT id, user_id, account_id, symbol, direction, status, 
    ...
    return res.rows.map(r => ({
      ...
      status: r.status as "OPEN" | "CLOSED" | "CANCELED",
  ...
  static async createTrade(userId: string, trade: Omit<Trade, "id" | "userId" | "createdAt" | "updatedAt">): Promise<Trade> {
    const pool = this.getPool();
    ...
    if (pool) {
      await pool.query(
        ...
        [
          ...
          trade.session || null,
          (trade as any).setup || null,
          trade.ruleAdherence !== false,
          (trade as any).notes || null,
  ...
  static async updateTrade(userId: string, tradeId: string, updates: Partial<Trade>): Promise<Trade | null> {
    const pool = this.getPool();
    if (!pool) return null;

    // Check ownership
    const check = await pool.query(`SELECT id FROM trades WHERE id = $1 AND user_id = $2;`, [tradeId, userId]);
    if (check.rows.length === 0) return null; // Unauthorized or not found
    ...
    if (updates.netPnl !== undefined) { setClauses.push(`net_pnl = $${idx++}`); values.push(updates.netPnl); }
    if (updates.netPnlPct !== undefined) { setClauses.push(`net_pnl_pct = $${idx++}`); values.push(updates.netPnlPct); }
    if (updates.ruleAdherence !== undefined) { setClauses.push(`rule_adherence = $${idx++}`); values.push(updates.ruleAdherence); }

    setClauses.push(`updated_at = NOW()`);
    values.push(tradeId, userId);

    await pool.query(
      `UPDATE trades SET ${setClauses.join(", ")} WHERE id = $${idx++} AND user_id = $${idx++};`,
      values
    );
  ...
  // =========================================================================
  static async getStrategies(userId: string): Promise<StrategyAST[]> {
    const pool = this.getPool();
    if (!userId || !pool) return [];

    const res = await pool.query(
      `SELECT id, user_id, name, direction, symbol, timeframe, category, 
  ...
  static async saveStrategy(userId: string, strategy: StrategyAST): Promise<StrategyAST> {
    const pool = this.getPool();
    if (pool) {
      await pool.query(
  ...
  static async deleteStrategy(userId: string, strategyId: string): Promise<boolean> {
    const pool = this.getPool();
    if (!pool) return false;
  ...
  // =========================================================================
  static async getTradingPlan(userId: string): Promise<TradingPlan> {
    const pool = this.getPool();
    if (pool && userId) {
      const res = await pool.query(
        `SELECT id, user_id, title, is_active, max_daily_loss_pct, max_risk_per_trade_pct, 
    ...
    if (pool && userId) {
      await pool.query(
        `INSERT INTO trading_plans (id, user_id, title, max_daily_loss_pct, max_risk_per_trade_pct, max_trades_per_day, allowed_sessions, allowed_markets, entry_criteria, exit_criteria, forbidden_rules)
  ...
  static async updateTradingPlan(userId: string, updates: Partial<TradingPlan>): Promise<TradingPlan> {
    ...
    if (pool) {
      await pool.query(
        `INSERT INTO trading_plans (id, user_id, title, is_active, max_daily_loss_pct, max_risk_per_trade_pct, max_trades_per_day, allowed_sessions, allowed_markets, entry_criteria, exit_criteria, forbidden_rules, updated_at)
  ...
  // =========================================================================
  static async getRiskSettings(userId: string): Promise<RiskSettings> {
    const pool = this.getPool();
    if (pool && userId) {
      const res = await pool.query(
        `SELECT id, user_id, max_account_risk_pct, daily_loss_limit_pct, max_open_positions, 
    ...
    if (pool && userId) {
      await pool.query(
        `INSERT INTO risk_settings (id, user_id, max_account_risk_pct, daily_loss_limit_pct, max_open_positions, trailing_stop_default, break_even_threshold, enforce_strict_risk)
  ...
  static async updateRiskSettings(userId: string, updates: Partial<RiskSettings>): Promise<RiskSettings> {
    ...
    if (pool) {
      await pool.query(
        `INSERT INTO risk_settings (id, user_id, max_account_risk_pct, daily_loss_limit_pct, max_open_positions, trailing_stop_default, break_even_threshold, enforce_strict_risk, updated_at)
  ...
  static async getJournalEntries(userId: string): Promise<JournalEntry[]> {
    ...
    const res = await pool.query(
      `SELECT id, user_id, trade_id, title, content, lessons, created_at, updated_at
  ...
  static async createJournalEntry(userId: string, entry: Omit<JournalEntry, "id" | "userId" | "createdAt" | "updatedAt">): Promise<JournalEntry> {
    const pool = this.getPool();
    ...
    if (pool) {
      await pool.query(
        `INSERT INTO journal_entries (id, user_id, trade_id, title, content, lessons, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8);`,
        [
          entryId,
          userId,
  ...
  static async deleteJournalEntry(userId: string, entryId: string): Promise<boolean> {
    const pool = this.getPool();
    if (!pool) return false;
  ...
  // =========================================================================
  static async getTradingAccounts(userId: string): Promise<TradingAccount[]> {
    const pool = this.getPool();
    if (!userId || !pool) return [];

    const res = await pool.query(
      `SELECT id, user_id, name, broker, account_type, currency, balance, equity, is_default, created_at, updated_at
  ...
  static async createTradingAccount(userId: string, account: Omit<TradingAccount, "id" | "userId" | "createdAt" | "updatedAt">): Promise<TradingAccount> {
    const pool = this.getPool();
  ...
  static async getSubscription(userId: string) {
    const pool = this.getPool();
    if (!userId || !pool) return { tier: "FREE", status: "ACTIVE" };

    const res = await pool.query(
      `SELECT id, user_id, tier, status, current_period_start, current_period_end, cancel_at_period_end, provider, created_at, updated_at
       FROM subscriptions
       WHERE user_id = $1 LIMIT 1;`,
      [userId]
    );
  ...
  static async upgradeSubscription(userId: string, tier: "PRO" | "ELITE", provider: string, paymentRef: string) {
    const pool = this.getPool();
      ...
      await client.query(
        `INSERT INTO subscriptions (id, user_id, tier, status, provider, current_period_start, current_period_end, updated_at)
         ...
         ON CONFLICT (user_id) DO UPDATE SET
           ...
           updated_at = NOW();`,
        [`sub_${userId}`, userId, tier, provider, thirtyDaysFromNow]
      );
